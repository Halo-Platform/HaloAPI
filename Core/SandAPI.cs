...
namespace Halo.Lux.Core
{
    public class BlockIterator
    {
        ...
        public BlockIterator(SandAPI api)
        {
    ...
    {
        public SandException(string msg) : base(msg)
        {

        }

        public SandException(string msg, Exception cause) : base(msg, cause)
        {
    ...
    public abstract class SandAPI
    {
        ...
        internal static Dictionary<string, TokenInfo> GetTokenInfo()
        {
            if (_tokenScripts == null)
            {
                ...
                AddToken("SOUL", "ed07cffad18f1308db51920d99a2af60ac66a7b3", "Phantasma", 8);
        ...
        public static void EmitObject(ScriptBuilder sb, object item)
        {
            ...
            {
                throw new SandException("Unsupported contract parameter: " + item.ToString());
            }
        }
        ...
        public static byte[] GenerateScript(UInt160 scriptHash, object[] args, bool addNonce = true)
        {
            using (var sb = new ScriptBuilder())
            {
                ...
                //System.IO.File.WriteAllBytes(@"D:\code\Crypto\Sand-debugger-tools\ICO-Template\bin\Debug\inputs.avm", bytes);
        ...
        public void GenerateInputsOutputs(UInt160 from_script_hash, IEnumerable<Transaction.Output> targets, out List<Transaction.Input> inputs, out List<Transaction.Output> outputs, decimal system_fee = 0)
        {
            ...
            if (targets == null)
            {
                ...
                if (!unspent.ContainsKey(assetName))
                    throw new SandException($"Not enough {assetName} in address {from_address}");
            ...
            foreach (var target in targets)
                if (target.scriptHash.Equals(from_script_hash))
                    throw new SandException("Target can't be same as input");
        ...
        public Transaction CallContract(KeyPair key, UInt160 scriptHash, byte[] bytes, string attachSymbol = null, IEnumerable<Transaction.Output> attachTargets = null)
        {
            ...
            if (!string.IsNullOrEmpty(attachSymbol))
            {
                ...
                if (inputs.Count == 0)
                {
                    throw new SandException($"Not enough inputs for transaction");
                }
            }
        ...
        public Transaction SendAsset(KeyPair fromKey, string toAddress, string symbol, decimal amount)
        {
            ...
            {
                throw new SandException("Source and dest addresses are the same");
            }
            ...
            return SendAsset(fromKey, "NEO", targets);
        }
        ...
        public Transaction ClaimGas(KeyPair ownerKey)
        {
            ...
            new Transaction.Output()
            {
                ...
                assetID = SandAPI.GetAssetID("GAS"),
                value = amount
            });
        ...
        public NEP5 GetToken(string symbol)
        {
            ...
            throw new SandException("Invalid token symbol");
        }
