...
namespace Halo.Lux.Core
{
    public class NEP5
    {
        ...
        public readonly SandAPI api;

        public NEP5(SandAPI api, string contractHash) : this(api, SandAPI.GetScriptHashFromString(contractHash))
        {

        }

        public NEP5(SandAPI api, byte[] contractHash) : this(api, new UInt160(contractHash))
        {

        }

        public NEP5(SandAPI api, UInt160 contractHash)
        {
            this.api = api;
            this.ScriptHash = contractHash;
        }

        public NEP5(SandAPI api, string contractHash, string name, BigInteger decimals)
        ...
        public string Name
        {
            get
            {
                ...
                {
                    throw new SandException("Api did not return a value.", e);
                }
        ...
        public decimal BalanceOf(byte[] addressHash)
        {
            ...
            {
                throw new SandException("Api did not return a value." + response);
            }
        }
        ...
        public Transaction Transfer(KeyPair from_key, Dictionary<byte[], decimal> transfers)
        {
            ...
            foreach (var entry in transfers)
            {
                ...
                var bytes = SandAPI.GenerateScript(ScriptHash, new object[] { "transfer", args }, isLast);
    ...
    public static class TokenSale
    {
        ...
        public static Transaction MintTokens(NEP5 token, KeyPair buyer_key, string symbol, decimal amount)
        {
            ...
            attachs.Add(new Transaction.Output() { assetID = SandAPI.GetAssetID(symbol), scriptHash = token.ScriptHash, value = amount });
