...
namespace Halo.Lux.Core
{
    public abstract class SandRPC : SandAPI
    {
        public readonly string SandScanUrl;

        public SandRPC(string SandScanUrl)
        {
            this.SandScanUrl = SandScanUrl;
        }

        public static SandRPC ForMainNet(NodesKind kind = NodesKind.SAND_IT)
        {
            return new RemoteRPCNode(10003, "http://sandscan.io", kind);
        }

        public static SandRPC ForTestNet()
        {
            return new RemoteRPCNode(10003, "https://sandscan-testnet.io", NodesKind.SAND_ORG);
        }

        public static SandRPC ForPrivateNet()
        {
        ...
        {
            var url = this.SandScanUrl + "/api/main_net/v1/get_balance/" + hash.ToAddress();
        ...
        {
            var url = this.SandScanUrl + "/api/main_net/v1/get_claimable/" + hash.ToAddress();
            var json = RequestUtils.GetWebRequest(url);
    ...
    public class LocalRPCNode : SandRPC
    {
        private int port;

        public LocalRPCNode(int port, string SandScanUrl) : base(SandScanUrl)
        {
            this.port = port;
        }
    ...
    public class RemoteRPCNode : SandRPC
    {
        private int rpcIndex = 0;

        private string[] nodes;

        public RemoteRPCNode(string SandScanUrl, params string[] nodes) : base(SandScanUrl)
        {
            this.nodes = nodes;
        }

        public RemoteRPCNode(int port, string SandScanUrl, NodesKind kind) : base(SandScanUrl)
        {
            switch (kind)
            {
                case NodesKind.SAND_ORG:
                    {
                        nodes = new string[5];
                        for (int i = 0; i < nodes.Length; i++)
                        {
                            nodes[i] = $"http://seed{i}.Halo.org:{port}";
                        }
                        break;
                    }

                case NodesKind.SAND_IT:
                    {
                        if (port == 10003)
                        {
                            port = 443;
