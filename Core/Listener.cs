...
namespace Halo.Lux.Core
{
    ...
    public class Snapshot: IBlockchainProvider
    {
        ...
        public Snapshot(UInt160 scriptHash, SandAPI api, uint startBlock, uint endBlock = 0)
        {
            if (endBlock == 0)
            {
        ...
        private void MergeTransaction(SandAPI api, Transaction tx)
        {
            transactions[tx.Hash] = tx;
        ...
        private static uint FindBlock(SandAPI api, uint timestamp, uint min, uint max)
        {
            var mid = (1 + max - min) / 2;
            do
            {
        ...
        public static uint FindBlock(SandAPI api, DateTime date)
        {
            uint min = 0;
        ...
        public void Execute(SandAPI api, UInt160 script_hash, Action<ListenerVM> visitor)
        {
