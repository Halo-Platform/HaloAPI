...
using System.Text;
using VMArray = Halo.Lux.VM.Types.Array;

namespace Halo.Lux.VM
{    
    [Flags]
            ...
            if (opcode >= OpCode.PUSHBYTES1 && opcode <= OpCode.PUSHBYTES75)
                ...
                switch (opcode)
                {
                    ...
                    case OpCode.HASH160:
                        {
                            byte[] x = context.EvaluationStack.Pop().GetByteArray();
                            context.EvaluationStack.Push(CryptoUtils.Hash160(x));
                        }
                        break;
                    case OpCode.HASH256:
                        {
                            byte[] x = context.EvaluationStack.Pop().GetByteArray();
                            context.EvaluationStack.Push(CryptoUtils.Hash256(x));
                        }
                        break;
                    case OpCode.CHECKSIG:
                        {
                            ...
                            try
                            {
                                context.EvaluationStack.Push(CryptoUtils.VerifySignature(ScriptContainer.GetMessage(), signature, pubkey));
                            }
                    ...
                    case OpCode.VERIFY:
                        {
                            ...
                            try
                            {
                                context.EvaluationStack.Push(CryptoUtils.VerifySignature(message, signature, pubkey));
                            }
                            catch (ArgumentException)
                            {
                                context.EvaluationStack.Push(false);
