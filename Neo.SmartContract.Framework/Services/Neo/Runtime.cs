...
namespace Neo.SmartContract.Framework.Services.Neo
{
    public static class Runtime
    {
        ...
        public static object Cast(global::System.Type type, object src)
        {
            ...
            {
                var num = ((Halo.Lux.VM.StackItem)src).GetBigInteger();
                return num;
            }

            var str = (Halo.Lux.VM.Types.Struct)src;
            ...
            foreach (var field in fields)
            {
                ...
                if (item is Halo.Lux.VM.Types.ByteArray)
                {
                    val = item.GetByteArray();
                }
                else
                if (item is Halo.Lux.VM.Types.Integer)
                {
                    val = item.GetBigInteger();
                }
                else
                if (item is Halo.Lux.VM.Types.Boolean)
                {
                    val = item.GetBoolean();
                }
                else
                if (item is Halo.Lux.VM.Types.Struct && ((Halo.Lux.VM.Types.Struct)item).Count == 0)
                {
                    val = new BigInteger(0);
                }
                else
                if (item is Halo.Lux.VM.Types.Array)
                {
                    var array = (Halo.Lux.VM.Types.Array)item;
